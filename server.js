...
io.on('connection', (socket) => {
    ...
    socket.on('init', (username) => {
        ...
        socket.emit('message', formatMessage('ADMIN', `Hoşgeldin ${user.username}, Ders içi sohbet etmenin ve hakaretin sorumluluğu kullanıcıya aittir.`));

        //broadcast all the users in the same room 
        socket.broadcast
            .to(user.room)
            .emit('message', formatMessage('ADMIN', `${user.username} Sohbete Katıldı`));
    ...
    socket.on('join', (room) => {
        ...
        socket
            .to(oldUser.room)
            .emit('message', formatMessage('ADMIN', `${oldUser.username} Sohbetten Çıktı`));
        ...
        socket.emit('message', formatMessage('ADMIN', `Odaya Hoşgeldin ${user.room} ${user.username}`));

        //broadcast all the users in the same room 
        socket.broadcast
            .to(user.room)
            .emit('message', formatMessage('ADMIN', `${user.username} Odaya Katıldı ${user.room}`,));
